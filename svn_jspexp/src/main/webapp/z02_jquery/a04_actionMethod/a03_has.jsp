...
# event.target : 이벤트가 일어난 대상 객체..
   ...
   $("선택자").이벤트명(function(event){
   		if($(event.target).is("선택자")) 이벤트가 일어난 요소객체가 특정 선택자일 때..
   });  : 익명이벤트핸들러 함수의 매개변수를 통해서
   	event의 매개값을 가져올 수 있다.
   ex)	
   	<input type="button" onclick="click01()"
   		==> 해당 버튼을 통해서 지금은 없지만, 앞으로 div 하위에 특정한 요소객체를 만들어 갈때,
   			이 앞으로 만들어질 요소객체에 대한 이벤트 처리시, 사용된다.
