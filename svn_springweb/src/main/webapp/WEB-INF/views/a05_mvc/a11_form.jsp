...
<div class="jumbotron text-center">
	...
	<h4>선택되어진 os</h4>
	<c:forEach var="cd" 
		items="${favoriteOsCodes}"> 
		${cd.code} - ${cd.label},
	</c:forEach><br>
	<c:forEach var="choice" 
		items="${formCommand.favoriteOs}"> 
		${choice}
	</c:forEach>	
...
<!-- form 하위에 mapping되는 VO객체의 property와 연결되기 위하여 선언 필요.
	 spring의 form 객체를 활용하면, 요청값을 따로 선언하지 않아도 된다.
	 ex)  <select name="dept">
	 		<option value="10">인사</option>
	 		<option value="20">재무</option>
	 		<option value="30">회계</option>
	 		<option value="40">총무</option>
	 	 <script>
	 	 	$("[name=dept]").val("${param.dept");
	 	 	스프링에서 지원하는 form태그를 활용하면 이런 처리가 필요없이 바로
	 	 	요청값에 의해 선택된 값을 처리해준다..
	 	 </sctipt>	
...
<form:form modelAttribute="formCommand">
  ...
  <div class="input-group mb-3">
    ...
    <form:select path="loginType[1]" items="${loginTypes}" 
    	 class="form-control" />
  </div>
  <div class="input-group mb-3">
    <div class="input-group-prepend">
  ...
  <div class="input-group mb-3">
    <div class="input-group-prepend">
 	...
 	<form:checkboxes items="${favoriteOsCodes}" 
 		path="favoriteOs"
