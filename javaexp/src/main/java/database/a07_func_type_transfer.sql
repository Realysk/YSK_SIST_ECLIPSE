 		...
 		2) 명시적 데이터 타입의 변환
 			일반적으로 데이터베이스는 변환함수를 이용하여 데이터 타입을 변환하여 효과적으로 처리한다.
 				1) to_char(문자열로,'형식') : 숫자/날짜 타입을 문자열로 변환해주는 함수이다.
 					ex) 숫자형/날짜형 타입을 원하는 형식으로 화면에 출력할 때 활용된다.
 				2) to_number(숫자로) : 날짜/문자 타입을 숫자로 변환해주는 함수이다.
 					ex) 문자형을 숫자로 변환해서 where 조건문이나 select에서는 다른 컬럼과 연산이 필요할 때 입력이나 수정에서는 문자열로 입력된 데이터를 숫자형으로 변환해서 등록할 때 활용된다.
 				3) to_date(날짜로,'형식') : 문자/숫자 타입을 날짜로 변환해주는 함수이다.
 					ex) 입력되는 데이터가 문자열형이고 특정한 형식으로 날짜형 타입의 컬럼에 입력할 때 사용된다.
 */
