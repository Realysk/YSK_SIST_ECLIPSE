 ...
 # 정규화
 	...
 	2. 목차
 		...
 		3) 정규화 과정 : 원자성 위배에 따른 컬럼 분리
 			...
 			제 5 정규형
 	3. 정규화와 이상 현상
 		1) 이상 현상 : 불필요한 *데이터 중복으로 인해 릴레이션(테이블)에 대한 데이터 삽입, 수정, 삭제 연산을 수행할 때 발생할 수 있는 부작용
...
/*
		 ...
		 비식별자 관계 : 참조하는 테이블의 키를 현재 테이블에 키로 사용하지 않을 때
		 	ex) emp의 deptno는 dept의 deptno와 비 식별자 관계
 	...
 	10. 보이스/코드 정규형 (BCNF:Boyce/Codd Normal Form)
 		1) 필요성
 			하나의 릴레이션(테이블)에 여러 개의 후보키(사원 테이블에 주민번호, 복합 된 후보키)가 존재하는 경우 제 3 정규형까지 모두 만족해도 이상 현상이 발생할 수 있다.
