...
public class A01_Basic {
	/*
	 ...
	 1. 객체를 재활용하고, 유지보수할 때, 다른 소스에 영향이 없으면 확장성 있는 프로그래밍(응집성 있는 프로그래밍)을 할 때,
	 ...
	 2. 상속의 기본 프로그래밍
	 	...
	 	class Son extends Father {
	 		Son 클래스는 [접근제어자범위 안]에서 상위 클래스의 구성요소를 사용할 수 있다.
	 ...
	 3. 상속의 종류
	 	1) 상위 실제 클래스 vs 하위 실제 클래스 상속
	 	2) 상위 추상 클래스 vs 하위 실제 클래스 상속
	 	3) 상위 인터페이스 vs 하위 실제 클래스 상속
	 	
	 4. 추상 클래스
	 	1) 오버라이딩 : 하위에서 상위 메서드 재정의 처리.
	 		overriding(재정의), polymorphism(다형성)
	 	2) 상속관계에서 상위에 body({}) 없는 메서드를 정의하면, 이 메서드는 abstract가 붙은 추상 메서드가 된다.
...
// Daughter : Mother을 상속 받아서 필드를 재활용하고,
