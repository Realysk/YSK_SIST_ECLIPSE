...
/*
 * ex) 코드 계층적 block 처리 아래의 Calcu 클래스를 선언하고 입력 값 2개로 사칙연산을 하는 코드를 처리하되 변수의 중괄호 선언
 * 규칙에 의해서 선언 후 할당하고 main()를 통해서 호출하세요.
 */

class Calcu {
	// 필드 선언
	// 1. 이 클래스에서 다 사용할 수 있는 전역변수 : class의 {}(중괄호)에서 선언되있음.
	...
	Calcu(int num03, int num04) {
		// int num01 : 매개변수로 받는 데이터는 지역변수이기에 다른 중괄호 블럭에서 사용할 수 없다.
