	...
	/*
	 # String
	 	...
	 	3. 객체이기 때문에 heap메모리 주소에 할당된다.
	 		1) 첫번째 방식 : 문자열만 같으면 같은 주소에 할당이 된다.
	 			String name1 = "홍길동";
	 			String name2 = "홍길동";
	 			String name3 = "홍길동";
	 			"홍길동"이라는 문자열은 heap영역에 동일한 주소를 참조하여 설정된다.
	 			직접 코딩에 의해서 할당이 되어 사용 가능하다.
	 			name1 == name2는 동일한 주소값이기 때문에 동일한 true로 처리가 된다.
	 			
	 		2) 두번째 방식 : 문자열이 각각, 다른 주소를 만들어서 문자열의 데이터를 가지고 온다.
...
// 			문자열만 비교해서 데이터를 처리할 때는 == 와 같은 stack 영역의 데이터비교 방식은 문제가 발생한다.
// 			## 자바에서 문자열의 데이터를 비교할 때는 stack영역의 주소값이 아닌 문자열 자체를 비교해주는
// 			.equals()메서드를 활용해야 한다.
