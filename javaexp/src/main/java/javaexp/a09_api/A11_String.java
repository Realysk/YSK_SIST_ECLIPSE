...
public class A11_String {
	...
	public static void main(String[] args) {
		...
		/*
		 # String 문자열의 시작
		 	1. 숫자
		 		...
		 		{'H', 'e', 'l', 'l', 'o'}
		 		==> {25,34,72,43,22} : 코드값으로 bytes 배열은 문자가 모인 문자열로 사용할 수 있다.
		 		==> 객체지향 프로그램에서 char[] 배열에서 한 단계 더 나아가 String 문자열 객체를 만들게 되었다. C++부터 사용되기 시작하였다.
		 */
		...
		// 숫자에 해당하는 문자(char)가 모여서 문자열을 이후게 한다는 개념 하에 String 생성자를 통해서 문자열을 만들 수 있게 했다.
...
//		ex) 위와 같이 byte 배열과 유니코드를 이용해서 Good job을 출력하세요.
		...
		*/
			...
			*/
			...
			// Switch(문자/문자열)도 가능
			switch (gender) {
//			case '1': 과 case 1: 은 천지차이
			case '1': // break가 없어 계속 밑으로
			case '3':
				System.out.println("주민번호 : " + ssn + " [" + ssn.charAt(7) + "]" + " => 남자");
				break;
			case '2': // break가 없어 계속 밑으로
			case '4':
				System.out.println("주민번호 : " + ssn + " [" + ssn.charAt(7) + "]" + " => 여자");
				break;
			default:
			...
			/*
				2) getBytes() : 문자열의 byte 배열을 가져온다.
					이 때, 매개변수값으로 EUC-KR이나 UTF-8로 한글 encoding 형식을 지정할 수 있다.
			*/
			...
			/*
			ex) "오늘도 즐겁고 감사한 하루!!" encoding으로 euc-kr과 utf-8로 처리하여 출력하세요.
			*/
			...
			System.out.println("문자열 4번째 : " + str4);
			try {
				byte[] bytes7 = str7.getBytes("EUC-KR");
				String str9 = new String(bytes7);
				System.out.println("문자열 5번째 : " + str9);
				byte[] bytes8 = str7.getBytes("UTF-8");
				String str10 = new String(bytes8);
				System.out.println("문자열 6번째 : " + str10);
			} catch (UnsupportedEncodingException e) {
				e.printStackTrace();
			}
			...
			if(msg.indexOf("짜장면") != -1) {
				...
				System.out.println("짜장면 해당 문자열을 발견하지 못했습니다.");
			}
			System.out.println("문자열의 크기 : " + msg.length());
			
			/*
			 ex) "메뉴는 사과, 바나나, 딸기입니다."
			 	선택할 과일을 입력하세요. (Scanner)
			 	indexOf를 입력한 과일이 없으면?
			 	-> "해당 과일은 메뉴에 없습니다."
			 	각 index 위치를 확인해서 1번째/2번째/3번째 과일임을 표시하세요.
			 	@@ 번째 과일 @@를 선택했습니다.
			 	
			 	대상문자열.indexOf(찾고자하는문자열) :
			 	결과로 찾고자 하는 문자열이 없으면 -1
			 	결과로 찾고자 하는 문자열이 있으면 해당 문자열의 시작 위치를 리턴한다.
			...
			System.out.println("메뉴는 사과, 바나나, 딸기입니다.\n");
			...
			System.out.println("선택한 index : " + findIdx);
				switch(findIdx) {
				case -1:
					System.out.println("해당 과일은 메뉴에 없습니다.");
					break;
				case 4:
					System.out.println("첫 번째 과일 사과를 선택했습니다.");
					break;
				case 8:
					System.out.println("두 번째 과일 바나나를 선택했습니다.");
					break;
				case 13:
					System.out.println("세 번째 과일 딸기를 선택했습니다.");
				}
			}
