		...
		/*
		 # 정리문제
		 	...
		 	1. static, final, static final의 각각의 차이점을 기본예제를 통해서 기술하세요.
		 		...
		 		static : 객체 공유 메모리영역. 즉, 클래스 단위 멤버를 사용할 때 활용된다.
		 		final : 필드에서는 상수 선언으로 객체마다 하나의 상수를 초기 선언하면 변경이 불가능하다.
		 		...
		 		class Bear {
		 			private int eat_cnt;
		 			public static int TOT_BREAD_CNT; // 이 클래스로부터 생성되는 모든 Bear 객체가 공유 메모리 영역 남은 빵의 갯수
		 			final String NAME;
		 			static final String KIND = "곰돌이클래스";
		 			Bear(String name) {
		 				this.NAME = name; // 객체마다 초기로 선언하여 변경이 불가능하게 할 때 사용
		 			}
		 			public void eat() {
		 	...
		 	2. Robot 클래스를 정의할 때 "기계로봇", 로봇의 종류, 로봇공유전력남은양, 처리할 기능을
		 		필드로 선언하여 해당 내용을 활용하는 부분을 위 문제에 선언한 keyword의 기능에 맞게 처리하세요.
		 		(생성자, 메서드 필드 선언을 적절하게 하세요.)
		 	...
		 	3. 접근제어자의 범위를 기술하고, 클래스명 앞에 붙는 접근제어자와 생성자 앞에 붙는 접근제어자가 main()에서
		 		어떻게 영향을 미치는지 예제를 통하여 기술하세요.
		 		
		 		private : 이 제어자로 선언된 클래스 멤버는 외부에 공개되지 않으며 외부에서는 직접 접근이 불가능하다.
		 				public getXXX(), public setXXX() 간접적 접근은 가능하다.
		 				X(default) : 기본적으로 접근제어자가 붙어 있는 않는 멤버는 같은 패키지의 클래스들만 접근하여 사용할 수 있다.
		 							ex) 클래스 선언 최상단의 package a00_exp; 동일할때.
		 		protected : 상속관계에 있는 때 외부 패키지의 있는 클래스가 접근하여 사용할 수 있게 선언한다.					
				public : 이 제어자로 선언된 클래스 멤버는 외부에 공개되며 해당 객체를 사용하는 프로그램 어디에서나 직접 접근이 가능하다.
				default : 클래스 멤버 접근 제어의 기본값으로 가지고 있으며 별도로 명시하고 있다.
				// 외부 패키지에서 선언할 클래스 호출
				// import 없이 특정한 패키지의 클래스 선언하거나 현재 패키지의 클래스와 같은 이름의 클래스를 호출할 때 패키지명과 함께 선언한다.
...
class Robot {
	...
	private final String KIND; // 각각의 로봇의 종류 (각각의 로봇의 종류 변경되지 않게 처리.
	...
	public void working(int usingpower) {
		...
		System.out.println(ROBOT_CLASSES + "분류로 되어 있는 " + KIND + "로봇이 일을 한다.");
	...
	public void attack(int usingpower) {
		...
		System.out.println(ROBOT_CLASSES + "분류로 되어 있는 " + KIND + "로봇이 공격을 한다.");
		System.out.println("소모 전력 : " + usingpower);
		REST_POWER -= usingpower;
		System.out.println("남은 전력 : " + REST_POWER);
	}
