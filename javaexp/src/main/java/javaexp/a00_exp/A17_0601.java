	...
	public static void main(String[] args) {
		...
		# 0601 정리문제 [JAVA] #
			...
			3. String 문자열이 만들어질 때까지 과정을 bit단위부터 기술해보세요.
				...
				char[], byte[]을 통해서 모인 배열이 객채형 타입으로 설정한 것이 String이다.
				byte [] bytes = {64,27,30};
				char [] chars = {'H', 'e', 'l'};
			...
			4. String 생성자를 통해, 알파벳 대문자 8자의 임의의 비밀번호를 만들려고 한다. 
	    		...
	    		*/
					...
					// 알파벳 대문자 코드 범위 : 65~90
					// (int)(Math.ramdom()*26+65)
					// 배열의 데이터 입력 : pass.length길이보다 index 1 적다.
					for(int idx=0; idx<pass.length; idx++) {
						// 실수형이므로 해당 byte에 넣기 위하여 typecasting 처리
			...
			5. 학번을 연도(2자리)학년(2자리)반(1자리)성별(F/M)로 만들어서 처리할 때, 
				...
				*/
					...
					// 삼항연산자 code01.charAt(5) == 'F' 코드값이 F인지 여부에 따라 true이면 "여자", 그렇지 않으면 "남자"를 할당.
					String gender = code01.charAt(5) == 'F'?"여자":"남자";
					System.out.println(yyyy + " 년도 입학, " + grade + "학년 " + class01 + "반 " + gender);
				/*
			...
			6. 요일별로 중국집에 오늘은 메뉴를 바꿀려고 한다.
	    		...
	    		*/
					String menu = "오늘은 요일! 추천 메뉴 : @@\n";
					menu += "월 짜장\n화 짬뽕\n수 탕수육\n목 우동\n금 볶음밥\n토 마라\n일 우탕\n";
					String[]weeks = {"월", "화", "수", "목", "금", "토", "일"};
					String[]menus = {"짜장", "짬뽕", "탕수육", "우동", "볶음밥", "마라토", "우탕"};
					// 임의의 요일과 메뉴를 처리하기 위해 0~6범위 임의 index값 가져오기
					int ranIdx = (int)(Math.random()*weeks.length);
					// 임의의 요일과 해당하는 요일의 메뉴 할당.
					String week = weeks[ranIdx];
					String cmenu = menus[ranIdx];
					// menu 문자열에서 요일이라는 부분은 선택된 요일로 변경
			...
			7. 다음과 같은 데이터를 이중 반복문으로 처리하여 구매 물픔을 리스트 하세요.
				(split(), StringTokenizer 둘 다 활용)
				...
				*/
					...
					while(st.hasMoreTokens()) {
						// /로 물건 단위 정보를 처리하여, split을 통해서 각 단위 데이터를 배열로 가져온다.
						// 사과&3000&2 ==> {"사과","3000","2"};
						String fruits[] = st.nextToken().split("&");
						// forEach 구문을 통하여 배열의 단위 데이터를 String fruit에 할당하면서 하나씩 데이터를 뽑아 \t (탭 간격)을 설정.
