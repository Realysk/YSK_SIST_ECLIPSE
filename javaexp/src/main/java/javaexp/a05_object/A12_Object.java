...
public class A12_Object {
	...
	/*
	 # 클래스의 구성요소 통합 연습
	 	...
	 	2. 연습예제
	 		1) 물건 클래스를 선언하되 사용되는 필드는 물건명 가격 갯수 생성자를 통해서 초기화(물건명 가격) 처리.
	 			기능메서드1 : 매개변수로 갯수를 처리하여 물건명 가격 갯수를 출력하고 총계를 리턴하여 처리.
	 		ex)
	 		2) 쇼핑몰 클래스를 선언하고
	 			- 기능 내용 : 로그인(아이디, 패스워드)
	 						물건 구매(물건명, 갯수)
	 						상품리스트보기() : 쇼핑몰에 상품리스트 보기 ArrayList<Product>
	 						물건상세보기() : 상품 중에 특정한 상품을 보기 Product
	 */
	...
	public static void main(String[] args) {
		...
		tot+=p1.buy(5);
		Product01 p2 = new Product01("바나나", 6000);
		tot+=p1.buy(2);
		...
		/*
		 ex) Schedule 필드 : 계획내용, 계획된시간 생성자를 통해서 필드값 초기화.
		 	메서드를 통해서 실제 실행 시간 execute(시간) 리턴값으로 계획 실행의 차
		 	일정계획 3개와 실행된 내용에 대하여 계획대비 실행 차이를 출력하세요.
		 */
//		int tot2 = 0;
