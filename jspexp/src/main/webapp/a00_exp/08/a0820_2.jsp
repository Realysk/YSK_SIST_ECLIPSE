...
5. 쿠키를 활용하여 영화를 예매하고, 최종 결재할 내용만 남기는 처리를 하세요.
	...
	3. 출력처리..
		...
		[]  제목     종류   금액  관객수
		[]1 title1  kind1 pay1 cnt1
		[]2 title2  kind1 pay2 cnt2
...
<%
	...
	if(cks!=null&&cks.length>1){
		for(Cookie ck :cks){
			if(ck.getName().equals("no")){
				no = Integer.parseInt(ck.getValue());
			}	
		}
	}
	log("no:"+no);
	...
	if(title!=null){
		no++;
		response.addCookie(new Cookie("no",""+no));
		response.addCookie(new Cookie("titl"+no, URLEncoder.encode(title,"utf-8")));
		response.addCookie(new Cookie("titl"+no, URLEncoder.encode(title,"utf-8")));
		response.addCookie(new Cookie("kind"+no, URLEncoder.encode(kind,"utf-8")));
		response.addCookie(new Cookie("pay_"+no, pay));
		response.addCookie(new Cookie("cnt_"+no, cnt));
