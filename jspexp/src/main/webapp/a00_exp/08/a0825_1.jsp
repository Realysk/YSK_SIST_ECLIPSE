...
<body>
    ...
    <h3 align="center">등록된 사원정보 갯수:${empList.size()}</h3>
  	<table  align="center" class="listTable">
		...
		<tr><td>${empList.get(1).empno}</td>
			<td>${empList.get(1).ename}</td>
			<td>${empList.get(1).job}</td></tr>		
		<tr><td>${empList.get(2).empno}</td>
			<td>${empList.get(2).ename}</td>
			<td>${empList.get(2).job}</td></tr>												
	</table>  
