	...
	<%--
		# JSP에서 요청 값 처리
			...
			3. 단계별 연습 예제
				...
				4) 한 페이지에 요청값 처리
					1) form, location을 통해서 현재 페이지를 다시 요청하면서 요청 값을 처리하는 경우가 있다. (주로 검색처리)
					2) 물리적으로는 JSP가 한 페이지이지만 놀리적으로는 두 페이지로 요청 값을 전달하고 요청 값을 받아서 처리하는 것이다.
					3) 핵심 처리
						request.getParameter()를 통해서 한 소스에서 처리 되기에 초기 화면에 대한 처리를 하고 요청 값을 넘겼을 때를 구분하여 처리한다.
						- null에 대한 처리 : 초기에는 null을 가지고 있기 때문에 처리되어야 한다.
				5) form method="post" 방식 처리
					- 전송 방식을 post로 하면 url에 요청값(key=value)가 사라지고 요청 페이지만 남게 된다.
						기본의 url을 통해 전송하는 방식은 get방식인데 전송하는 용량 제한과 url에 보이는 단점이 있다.
						요청 값은 내부적으로 요청 body에 전달되어 요청 값을 보이지 않게 처리하거나 많은 요청을 처리할 때 활용된다.
