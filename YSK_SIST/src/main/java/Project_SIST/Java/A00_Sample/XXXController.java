...
/*
 화면단에서 오는 데이터를 받고, 화면단에 전달할 데이터를 설정하는 기능
 게시판 검색
 물건명 [  ] 가격 [ 1000 ] ~ [ 2000 ] [검색]
 물건명 가격 갯수 기타
 @@@	@@@ @@@ @@
 ...
 출력 : 물건명 리스트 - Model 객체에 key, value 형식으로 할당 처리
				마지막 매개변수로 선언해놓는다.
 select *
 from product
 where name like '%' ||'물건명'||'%'
...
1. 개발 패턴 3가지
 ...
 Controller => Service => Dao (DB 처리)
 			<=		  <=
 Controller => Service (DB 처리가 필요 없고 입력을 통해서 반복이나 조건 처리)
 			<= 
 Controller => Dao (X)
 			<=
 */
	...
	//
	public String schList2(ProductSch p, Model d) {
